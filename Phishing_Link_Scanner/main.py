...
test_urls = ['http://example.co', 'http://eXample.com', 'https://www.google.security-update.com', 'http://faceb00k.com/Login', 'https://google.com']
...
def is_phishing_url(url, legitimate_domains):
    subdomain, domain, suffix=extract_domain_parts(url)       # chec k if it's a known legitimate domain
    if f"{domain}.{suffix}" in legitimate_domains:
        return False                                          # check for misspelled domain names
    if is_misspelled_domain(domain, legitimate_domains):
        print(f"Potential phishing detection:  {url}")
        return True                                           # You can add more checks here, like suspicious subdomains
    return False                                              # Press the green button in the gutter to run the script.
